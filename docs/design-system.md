...
> カラーパレットは **ブラックベース（ライトモード）+ Blue アクセント** に統一しています。Claude Code に `docs/` フォルダごと渡せば自動参照されます。

---

## 1. カラーパレット（Black Primary / Light Mode）

### Primary - メインブランドカラー（Black / Charcoal）

| Token | HEX | 用途 |
|---|---|---|
| `--color-primary-50` | `#fafafa` | 背景のうすいアクセント、hover状態 |
| `--color-primary-100` | `#f5f5f5` | バッジ背景、選択状態の背景 |
| `--color-primary-200` | `#e5e5e5` | ボーダー（アクティブ）、テキスト選択 |
| `--color-primary-300` | `#d4d4d4` | ステップ番号（装飾的）、ドット |
| `--color-primary-400` | `#a3a3a3` | グラデーション末端、セカンダリテキスト |
| `--color-primary-500` | `#737373` | サブテキスト、キャプション |
| `--color-primary-600` | `#525252` | ボディテキスト（やや強め） |
| `--color-primary-700` | `#404040` | サイドバーのアクティブテキスト |
| `--color-primary-800` | `#262626` | 強調テキスト、CTA ボタン背景 |
| `--color-primary-900` | `#171717` | 最強調テキスト、ヒーロータイトル |

### Accent - 補助カラー（Blue）

| Token | HEX | 用途 |
|---|---|---|
| `--color-accent-50` | `#eff6ff` | アクセント背景、Hands-on ブロック背景 |
| `--color-accent-100` | `#dbeafe` | バッジ背景、Tip ボックス背景 |
| `--color-accent-200` | `#bfdbfe` | アクティブボーダー |
| `--color-accent-400` | `#60a5fa` | アクティブインジケーター、ダッシュドボーダー |
| `--color-accent-500` | `#3b82f6` | リンク色、タグ、アクティブバー |
| `--color-accent-600` | `#2563eb` | リンクホバー、ボタンアクセント |
...
| クラス | 効果 | 用途 |
|---|---|---|
| `text-gradient` | `bg-clip-text text-transparent` + primary（黒→チャコール）のリニアグラデーション | ヒーローの強調テキスト |
...
| アクセントボーダー | `border border-accent-200` | アクティブ / ループ要素 |
| ダッシュドボーダー | `border border-dashed border-accent-400` | ハンズオンブロック |
| 左ボーダー | `border-l-2 border-primary-800` | キーメッセージバナー |
...
| `shadow-lg shadow-black/8` | プロンプトカードホバー |
...
.text-gradient {
  ...
  background-image: linear-gradient(135deg, var(--color-primary-900), var(--color-primary-600));
}
```
用途: ヒーローの強調ワード
...
<motion.div
  ...
  className="w-[3px] h-4 rounded-full bg-primary-900"
...
- ナビリンク: `px-3 py-2 rounded-lg text-[13px] font-medium`
  ...
  - アクティブ: `bg-primary-50 text-primary-900 font-medium` + `layoutId` インジケーター
...
- ループステップ: `border-accent-200 bg-accent-50/40`
- ステップ番号: `text-2xl font-black tabular-nums` + `text-gray-200` or `text-accent-400`
- LOOPバッジ: `text-[9px] font-bold text-accent-500 bg-accent-50 border border-accent-200 px-2 py-0.5 rounded-full`
...
- ホバー: `hover:border-gray-300 hover:shadow-lg hover:shadow-black/8`
- ラベル: `text-accent-600 bg-accent-50 px-2.5 py-1 rounded-full` + `Sparkles` アイコン
...
- `pl-5 border-l-2 border-primary-800`
...
- 外枠: `rounded-xl border border-dashed border-accent-400 bg-accent-50/50`
...
- 回答: 左にインデント `pl-6` + `bg-accent-50/50 rounded-2xl rounded-tl-sm`
- アバター: `w-9 h-9 rounded-full` + イラストPNG
  - 質問者: `bg-gray-50 border-gray-200`
  - 回答者: `bg-accent-50 border-accent-100`
...
- リスト: アクセント色のドット `text-accent-400`
- インラインコード: `px-1.5 py-0.5 rounded bg-gray-100 text-primary-800 text-xs font-mono`
...
- グリッド線: `opacity-[0.04]`, 40px間隔, primary色（黒）ベース
- グラデーションオーブ: 2つ、`radial-gradient` で `opacity-20`, 20-25秒で浮遊
...
```
- 番号バッジ: `w-8 h-8 rounded-lg bg-primary-100 text-primary-900 text-sm font-bold`
...
```
- `px-1.5 py-0.5 rounded bg-accent-50 text-accent-600 text-xs font-mono font-medium border border-accent-100`
...
<div className="relative p-5 rounded-xl border border-gray-200 bg-white overflow-hidden">
  ...
  <span className="inline-block mt-3 text-[10px] font-bold text-accent-500 bg-accent-50 px-2 py-0.5 rounded">
    Ch.1-5
  </span>
  {/* 右下の装飾イラスト */}
...
```
text-[11px] px-2.5 py-1 rounded-full border border-gray-200 text-gray-500 hover:border-gray-400 hover:text-primary-900 hover:bg-primary-50 transition-all
```
...
`--color-primary-*` は Black/Charcoal 系、`--color-accent-*` は Blue 系。
Primary を差し替えればブランドカラーが変わり、Accent を差し替えればリンクやハイライト色が変わる。
Gray, Surface はそのまま流用可能。
